...
st.title("AIRBNB DATA ANALYSIS | By Vanmathi R")
st.write("")   
...
with st.sidebar:
    select = option_menu("Topics", ["Home", "Data Exploration", "Explore"],
    default_index=2,
...
if select == "Home":
    image1= Image.open("C:\Vanmathi DS\Capstone Project\Project3_Airbnb Analysis\Airbnb Pic.png")
    st.image(image1)

    st.header("Glimpse Of Airbnb:")
    st.write("")
    st.write('''***Airbnb is a web-based marketplace that allows property owners to rent out their spaces to travelers.***''')
    st.write("")
    st.header("Features:")
    st.write("")
    st.write(''' ***Unique stays***''')
    st.write(''' ***Experience***''')
    st.write(''' ***Adventure***''')
...
if select == "Data Exploration":
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Analysis Of The Price",
    ...
    with tab2:
        ...
        def dataframe():
            df_a= pd.read_csv(r"C:\Users\Test\Desktop\PHONEPE\Airbnb.csv")
            return df_a

        df_a= dataframe()

        st.title("**Analysis Of Availability**")
        col1,col2= st.columns(2)
    ...
    with tab3:
        ...
        def datafr():
            df= pd.read_csv(r"C:\Users\Test\Desktop\PHONEPE\Airbnb.csv")
            return df
...
if select == "Explore":
    st.header("WRAP UP OF THE PROJECT")

    st.subheader(":blue[Data Collection:]")

    st.write('''***Gather data from Airbnb's public API or available sources.***''')
    
    st.subheader(":blue[Data Cleaning & Preprocessing:]")
    ...
    st.write('''***Create visualizations to represent key metrics and trends.
        Using charts, graphs, and maps to convey information effectively.***''')
    
    st.subheader(":blue[Geospatial Analysis:]")
